...
describe("Menu", () => {
    ...
    it("calls onClick and closes menu", () => {
        ...
        render(<Menu trigger={<button>Open</button>} items={items.map((item) => item.value === "edit" ? { ...item, onClick } : item)}/>);
        fireEvent.click(screen.getByText("Open"));
    ...
    it("does not call onClick for disabled item", () => {
        const onClick = jest.fn();
        render(<Menu trigger={<button>Open</button>} items={items.map((item) => item.value === "disabled" ? { ...item, onClick } : item)}/>);
        fireEvent.click(screen.getByText("Open"));
